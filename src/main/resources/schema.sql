...
CREATE TABLE IF NOT EXISTS requests (
    ...
    REFERENCES request_priorities(request_priority_id),
    ...
    REFERENCES request_categories(request_category_id),
    CONSTRAINT fk_request_for_id
    FOREIGN KEY (request_for_id)
    REFERENCES request_for(request_for_id)
    );
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_trigger
        WHERE tgname = ''before_insert_requests''
    ) THEN
